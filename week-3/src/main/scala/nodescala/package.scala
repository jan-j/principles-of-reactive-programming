...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      case Nil => Future(Nil)
      case g :: gs => g.flatMap(t => all(gs).flatMap(ts => Future(t :: ts)))
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete(t => p.tryComplete(Try(cont(f))))
      p.future
    }
  ...
  object Subscription {
    /** Given two subscriptions `s1` and `s2` returns a new composite subsc?ription
