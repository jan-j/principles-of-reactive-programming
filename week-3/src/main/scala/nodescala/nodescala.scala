...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val serverComputation = Future.run() { ct =>
      async {
        while (ct.nonCancelled) {
          val exchange = await(listener.nextRequest())
          respond(exchange._2, ct, handler(exchange._1))
        }
      }
    }

    Subscription(listenerSubscription, serverComputation)
  }
